...
final class AppSlotsRowView: NSView {
    ...
    override init(frame frameRect: NSRect) {
        ...
        for index in 0..<AppSlotStore.slotCount {
            ...
            cell.index = index
            cell.row = self
            cell.onDragging = { [weak self] targeted in
                self?.onDragging?(targeted)
            }
            addSubview(cell)
            cells.append(cell)
        }
...
final class AppSlotCellView: NSView, NSDraggingSource {
    ...
    func reload(app: PinnedApp?, highlighted: Bool) {
        ...
        if let app {
            ...
            toolTip = "Drop an app here, or click to choose"
        }
        needsLayout = true
    }
    ...
    override func mouseEntered(with event: NSEvent) {
        setHighlighted(true)
        peek?.hover(app, isInside: true)
    }

    override func mouseExited(with event: NSEvent) {
        setHighlighted(false)
        peek?.hover(app, isInside: false)
    }
