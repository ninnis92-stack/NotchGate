...
struct PricingView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("A one-time unlock for calendar, weather, live network, disk and battery meters, and accent themes. No subscription.")
                .font(.system(size: 13))
    ...
    private var comparisonTable: some View {
        VStack(spacing: 0) {
            ...
            row("Music controls", free: true, pro: true)
            row("Auto-hide", free: true, pro: true)
            row("File shelf", free: false, pro: true)
            row("AirDrop sharing", free: false, pro: true)
            row("Calendar & weather", free: false, pro: true)
            row("Live network", free: false, pro: true)
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Product ID \(LicenseManager.productID). Sign the Mac app with your paid Apple Developer team. Local Debug uses the StoreKit file; Archive talks to App Store Connect.")
                .font(.system(size: 11))
