...
final class SystemMonitor {
    ...
    private func readProcesses() {
        ...
        var nextTicks: [pid_t: UInt64] = [:]
        var listed: [MonitoredProcess] = []
        listed.reserveCapacity(apps.count)
        for app in apps {
            let pid = app.processIdentifier
            var info = proc_taskinfo()
            let size = MemoryLayout<proc_taskinfo>.stride
            let got = proc_pidinfo(pid, PROC_PIDTASKINFO, 0, &info, Int32(size))
            guard got == Int32(size) else { continue }
            let ticks = UInt64(info.pti_total_user) + UInt64(info.pti_total_system)
            nextTicks[pid] = ticks
            let cpu: Double
            if let previous = previousCPUTicks[pid] {
                let delta = ticks >= previous ? ticks - previous : 0
                let interval = max(refreshInterval, 0.2)
                cpu = min(Double(coreCount) * 100, (Double(delta) / (interval * 1_000_000_000)) * 100)
            } else {
                cpu = 0
            }
            listed.append(
        ...
        previousCPUTicks = nextTicks
        processes = listed.sorted { lhs, rhs in
            if lhs.cpu == rhs.cpu { return lhs.memory > rhs.memory }
    ...
    private func readDevices() {
        ...
        for (index, screen) in NSScreen.screens.enumerated() {
            ...
            items.append(ConnectedDevice(id: "display-\(index)", kind: .display, name: name.isEmpty ? "Display \(index + 1)" : name))
        }
        items.append(contentsOf: Self.usbDeviceNames().prefix(8).map {
            ConnectedDevice(id: "usb-\($0)", kind: .usb, name: $0)
        })
        connectedDevices = items
    }

    private static func usbDeviceNames() -> [String] {
