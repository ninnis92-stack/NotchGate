    ...
    private var expandedBody: some View {
        VStack(spacing: 8) {
            ...
            if layout.showAppSlots {
                AppSlotsView(store: apps, onDragging: onDragTargeted)
            }

            if layout.showProcesses, license.isPro {
                FlyoutAnchor(kind: .processes, controller: flyout) {
                    ProcessStrip(stats: stats)
                }
            }

            if layout.showDevices, license.isPro {
                FlyoutAnchor(kind: .devices, controller: flyout) {
                    DeviceStrip(stats: stats)
                }
            }
