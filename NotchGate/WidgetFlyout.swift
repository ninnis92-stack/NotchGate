...
enum FlyoutKind: Equatable {
    ...
    var size: NSSize {
        ...
        case .processes: return NSSize(width: 400, height: 320)
        case .devices: return NSSize(width: 360, height: 220)
    ...
    var title: String {
        ...
        case .processes: return "Active Apps"
...
final class WidgetFlyoutController {
    ...
    func contains(_ point: NSPoint) -> Bool {
        guard isVisible, let panel, panel.isVisible else { return false }
        return panel.frame.contains(point)
    }

    func bridgeContains(_ point: NSPoint, island: NSRect) -> Bool {
        guard isVisible, let panel, panel.isVisible else { return false }
        let flyout = panel.frame
        let gap = island.minY - flyout.maxY
        guard gap > 0, gap < 28 else { return false }
        let bridge = NSRect(
            x: min(island.minX, flyout.minX),
            y: flyout.maxY,
            width: max(island.maxX, flyout.maxX) - min(island.minX, flyout.minX),
            height: gap
        )
    ...
    func dismiss() {
        ...
        hide(animated: false, notify: false)
    }

    private func scheduleShow(_ kind: FlyoutKind) {
        hideWork?.cancel()
        hideWork = nil
        showWork?.cancel()
    ...
    private func scheduleHide() {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            self.hideWork = nil
            guard !self.pointerInAnchor, !self.pointerInFlyout else { return }
            self.hide(animated: false, notify: true)
        }
        hideWork = work
    ...
    private func show(_ kind: FlyoutKind) {
        ...
        let panel = makePanel()
        panel.alphaValue = 1
        panel.contentView = FirstMouseHostingView(rootView: flyoutView(for: kind))
        panel.setFrame(frame(for: kind), display: true)
        OverlayChrome.apply(panel)
        panel.orderFrontRegardless()
        if kind == .search {
    ...
    private func hide(animated: Bool, notify: Bool) {
        let wasVisible = kind != nil
        kind = nil
        panel?.alphaValue = 1
        panel?.orderOut(nil)
    ...
    private func frame(for kind: FlyoutKind) -> NSRect {
        let size = kind.size
        let notch = notchPanel?.frame ?? NotchGeometry.current().frame(expanded: true)
        return NSRect(
            x: notch.midX - size.width / 2,
            y: notch.minY - size.height + 2,
            width: size.width,
            height: size.height
        )
    }

    private func flyoutView(for kind: FlyoutKind) -> some View {
        WidgetFlyoutView(
            kind: kind,
...
struct FlyoutAnchor<Content: View>: View {
    ...
    var body: some View {
        content()
            ...
            .onHover { hovering in
                controller.hoverAnchor(kind, isInside: hovering)
            }
            .onTapGesture {
                controller.reveal(kind)
            }
    }
}
...
private struct ProcessFlyout: View {
    var stats: SystemMonitor
            ...
            VStack(spacing: 6) {
                ForEach(stats.processes.prefix(8)) { process in
                    HStack(spacing: 8) {
                        ...
                        Text(ByteCountFormatter.string(fromByteCount: Int64(process.memory), countStyle: .memory))
                            .font(.system(size: 11, weight: .medium, design: .rounded))
                            .foregroundStyle(.white.opacity(0.45))
...
private struct DeviceFlyout: View {
    var stats: SystemMonitor

    var body: some View {
        if stats.connectedDevices.isEmpty {
            flyoutEmpty(symbol: "cable.connector", text: "No extra displays or USB devices reported.")
        } else {
            ScrollView {
                VStack(spacing: 6) {
                    ForEach(stats.connectedDevices) { device in
                        HStack(spacing: 8) {
