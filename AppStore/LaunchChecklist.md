...
- [x] App Sandbox **on** (user-selected files, calendar, location, Bluetooth, USB, network client, Apple Events for Music/Spotify)
- [x] Adversarial unit tests: `NotchGateTests` (Product → Test)
...
- [x] Screenshots regenerated from build 4 in `AppStore/Screenshots/*-1280x800.png`; upload copies are in `AppStore/Upload-to-App-Store-Connect/`
- [ ] Remove or replace previously uploaded screenshots that show the old capture shoulder, capture settings, “screenshots” pricing row, or drop shelf. The Pomodoro image remains accurate.
...
- [x] `Product → Test` (4 adversarial tests passed after capture and drop-shelf removal)
- [x] `Product → Archive` (final local build 4: `/tmp/NotchGate-1.0-build4-final.xcarchive`)
- [x] App Sandbox is **on**.
- [x] Previous build upload succeeded 2026-09-04
- [ ] Build 4 has intentionally not been uploaded pending final manual checks
...
NotchGate is a macOS utility with a Dock icon and an overlay at the camera notch. Hover the island to expand. Pro is a one-time IAP. When Pro is active, NotchGate requests Calendar and Location permission once at launch if their widgets are enabled. The widget flyouts are glance-only and do not initiate permission prompts.
