...
5. Localization en-US:
   ...
   - Description: One-time unlock for calendar, weather, live network, per-app load, devices, disk and battery meters, and accent themes.
6. Submit with the app version (Ready to Submit)

## Version 1.0
1. App icon 1024 from `AppStore/NotchGateIcon_1024.png`
2. Screenshots: `AppStore/Screenshots/*-1280x800.png` (from live captures)
...
6. Replacement build 4 (capture and drop-shelf features removed, dedicated Dock icon): `/tmp/NotchGate-1.0-build4-final.xcarchive`. Do not upload until the remaining manual checks pass. Then upload it, select build 4, attach IAP `com.notchlens.pro`, and resubmit for review.

## Review setup
When Pro is active, the app requests Calendar and Location permission once at launch if those widgets are enabled. No login or sample files are required. The widget flyouts are glance-only.

External services: Apple StoreKit handles the one-time Pro purchase, and Open-Meteo supplies weather forecasts using approximate coordinates only after the user grants Location access. NotchGate has no accounts, analytics, advertising, or tracking.
