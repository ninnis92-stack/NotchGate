# NotchGate — App Store listing copy

Paste these fields into App Store Connect. Mac app, not iPad. Screenshot sizes in this folder are **1280×800** and **2560×1600**.

## Name
NotchGate

## Subtitle (30 characters max)
Command center for the notch

## Promotional text (170 characters, optional)
Always-on island at the camera: CPU, Now Playing, apps, Pomodoro, and HUDs. Unlock Pro once for calendar, weather, and live stats.

## Description
NotchGate is a command center that lives on the MacBook camera housing. It stays visible in Full Screen. Hover the island to expand. Hover the top of the display only if something covered it.

Free
• Collapsed island with glance widgets (CPU/memory, clock, Pomodoro, and more)
• Now Playing with volume
• App slots and search
• Pomodoro timer (analog clock, countdown, draggable window)
• Volume and brightness HUD under the island

NotchGate Pro — $9.99 one time, no subscription
• Calendar next event
• Local weather
• Live network, per-app load, and devices
• Disk and battery meters
• Accent themes (Midnight, Aurora, Ember, Glacier)

NotchGate is built as a utility overlay, not a Dynamic Island clone. Search is ⌘⇧F.

## Keywords (100 characters max, comma-separated)
notch,hud,overlay,pomodoro,cpu,now playing,menu bar,macbook,timer,calendar

## Category
Primary: Utilities  
Secondary: Productivity

## Age rating
4+

## What's New (1.0)
First release: always-on notch command center, Pomodoro, HUDs, and optional Pro stats.

## Support URL
https://ninnis92-stack.github.io/notchgate-legal/support.html

## Marketing URL
Leave blank until you have a product site.

## Privacy Policy URL
https://ninnis92-stack.github.io/notchgate-legal/

## Copyright
2026 NotchLens

## SKU
notchgate

## Bundle ID
NotchLens.NotchGate

## IAP
Product ID: com.notchlens.pro  
Type: Non-Consumable  
US price: $9.99  
Display name: NotchGate Pro
